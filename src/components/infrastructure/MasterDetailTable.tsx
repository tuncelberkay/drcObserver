...
export function MasterDetailTable({ widget, config, previewData }: MasterDetailTableProps) {
  ...
  return (
    <div className="w-full bg-slate-900 border border-slate-800 rounded-xl flex flex-col overflow-hidden max-h-[800px]">
      {/* Title block */}
      <div className="flex items-center justify-between p-4 border-b border-slate-800 bg-slate-950/50">
         ...
         <div className="px-3 py-1 rounded bg-slate-900 border border-slate-800 text-[10px] text-slate-400 font-mono tracking-widest uppercase flex items-center gap-2">
      ...
      <div className="flex-1 overflow-x-auto overflow-y-auto custom-scrollbar">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-950/80 text-slate-400 text-[11px] uppercase tracking-widest border-b border-slate-800 sticky top-0 z-10 backdrop-blur-md">
              ...
              return (
                <React.Fragment key={rowId}>
                  <tr 
                    ...
                    className={`cursor-pointer transition-colors text-slate-200 hover:bg-indigo-500/5 ${isExpanded ? 'bg-slate-800/20' : ''}`}
                  >
                        ...
                        <div className="p-6 animate-in slide-in-from-top-2 duration-300">
                          <div className="space-y-4 col-span-full">
                            <h4 className="flex items-center gap-2 font-bold text-slate-200 text-sm">
                              <Terminal className="w-4 h-4 text-indigo-400" /> Detail Inspector (Key: {rowId})
                            </h4>
                            <div className="bg-slate-900/50 rounded-xl p-6 font-mono text-sm text-slate-400 border border-slate-800/80 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-6 gap-x-8 shadow-inner">
                               {childCols.map((col: string) => {
                                  ...
                                  return (
                                     <div key={col} className="flex flex-col gap-1.5 border-b border-slate-800/50 pb-3">
                                        <span className="text-[10px] uppercase tracking-widest text-indigo-300/80 font-bold">{col}</span>
                                        <span className="text-slate-200 truncate" title={renderVal}>{renderVal}</span>
                                     </div>
                                  )
                               })}
      ...
      <div className="flex items-center justify-between p-3 border-t border-slate-800 bg-slate-950/80 backdrop-blur-md">
         <div className="flex items-center gap-3">
             <div className="text-[10px] text-slate-400 font-mono tracking-widest uppercase flex items-center gap-1">
                Showing <span className="text-indigo-400 font-bold ml-1">{(currentPage - 1) * rowsPerPage + (metrics.length > 0 ? 1 : 0)}</span> - <span className="text-indigo-400 font-bold">{Math.min(currentPage * rowsPerPage, metrics.length)}</span> of <span className="text-slate-200">{metrics.length}</span>
             </div>
             {metrics.length > 0 && (
                 <select 
                    value={rowsPerPage} 
                    ...
                    className="bg-slate-900 border border-slate-700 text-slate-300 text-[10px] rounded px-1.5 py-0.5 outline-none focus:border-indigo-500 cursor-pointer"
                 >
         ...
         <div className="flex items-center gap-1.5">
            <button 
               ...
               className="p-1 rounded-md bg-slate-900 border border-slate-800 text-slate-300 hover:bg-slate-800 hover:text-indigo-400 hover:border-indigo-500/30 disabled:opacity-30 disabled:pointer-events-none transition-all"
            >
               <ChevronLeft className="w-4 h-4" />
            </button>
            <div className="px-2 text-[10px] font-mono font-bold text-slate-500">
