        ...
        {layoutType === "LIST" && (
           <div className="flex flex-col gap-6">
             {widgets.map((widget: any) => {
               ...
               if (!WidgetComponent) return <div key={widget.id} className="text-red-500">Missing CMS Component: {widget.componentKey}</div>
               
               let config = {}
               try { config = JSON.parse(widget.configJson) } catch (e) {}

               return <WidgetComponent key={widget.id} config={config} widget={widget} />
             })}
           </div>
        )}
                ...
                return (
                  <div key={widget.id} data-grid={{ i: widget.id, x: widget.x, y: widget.y, w: widget.w, h: widget.h }} className="h-full relative group">
                     ...
                     <div className="w-full h-full pointer-events-auto">
                        <WidgetComponent config={config} widget={widget} />
                     </div>
                  </div>
                )
             })}
