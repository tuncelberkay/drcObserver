...
export function DynamicPageRenderer({ pageData }: { pageData: any }) {
  ...
  const synchronizedWidgets = React.useMemo(() => {
    ...
    return widgets.map((w: any) => ({
      ...w,
      calculatedHeight: rowHeights[w.y],
      rowSync: rowMeta[w.y]
    }))
  }, [widgets])
        ...
        <div className="w-full relative">
          ...
          >
             {synchronizedWidgets.map((widget: any) => {
                ...
                // Use widget.calculatedHeight instead of widget.h to guarantee the strict row-level synchronization constraint!
                return (
                  <div key={widget.id} data-grid={{ i: widget.id, x: widget.x, y: widget.y, w: widget.w, h: widget.calculatedHeight }} className="h-full relative group">
